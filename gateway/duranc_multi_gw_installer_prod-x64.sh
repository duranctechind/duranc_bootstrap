...
#  with "pull access denied for durancai/production-installer-web-gw, repository
...
cat <<'NOTICE'
  ...
  curl -fsSL -o $HOME/duranc_gw_installer.sh "https://raw.githubusercontent.com/duranctechind/duranc_bootstrap/master/gateway/duranc_multi_gw_installer_new-x64.sh?cb=$(date +%s)" && chmod +x $HOME/duranc_gw_installer.sh && sudo bash $HOME/duranc_gw_installer.sh --env prod
